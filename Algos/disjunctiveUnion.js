...
function symmetricDifferences(numsA, numsB) {
  // CREATE A CONST VAR AND SET IT TO AN EMPTY ARR
  const myArr = []
  
  // IF BOTH ARRAYS ARE EMPTY RETURN MYARR
  if (numsA.length < 1 && numsB.length < 1) {
    return myArr
  }

  // ITERATE THROUGH NUMSA
  for (let i = 0; i < numsA.length; i++) {
    // CHECK TO SEE IF NUMSB VALUE EXISTS IN NUMSA VALUE 
    // AND ALSO CHECK IF NUMSA VALUE EXISTS IN MYARR, IF NOT PUSH NUMSA VALUE
    if (numsB.includes(numsA[i]) !== true && myArr.includes(numsA[i]) !== true) {
      myArr.push(numsA[i])
    }
  }

  // ITERATE THROUGH NUMSB
  for (let j = 0; j < numsB.length; j++)
    // CHECK TO SEE IF NUMSA VALUE EXISTS IN NUMBS VALUE 
    // AND ALSO CHECK IF NUMSB VALUE EXISTS IN MYARR, IF NOT PUSH NUMSB VALUE
    if (numsA.includes(numsB[j]) !== true && myArr.includes(numsB[j]) !== true) {
      myArr.push(numsB[j])
    }
  return myArr
}
