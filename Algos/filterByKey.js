...
function functionalFilterByKey(items, searchFor, searchBy) {
    // create a const variable and assign an empty arr
    const results =[]
    // iterate through the items
    for (let i = 0; i < items.length; i++) {
        // create a match variable and set to true
        let match = true
        // iterate through searchFor and check to see if searchFor matches searchBy
        for (let j = 0; j < searchFor.length; j++) {
            // if it doesn't match then break
            if (searchFor[j] !== items[i][searchBy][j]) {
                match = false
                break
            }
        }
        // outside the nested for loop if matched, then push item at i
        if (match === true) {
